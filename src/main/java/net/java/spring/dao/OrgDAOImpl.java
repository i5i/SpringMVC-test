...
public class OrgDAOImpl implements OrgDAO {
	...
	sessionFactory.getCurrentSession()
	    .delete(sessionFactory.getCurrentSession().get(Org.class, orgid));
	}
	...
	public List<Org> list(String sortby, Boolean asc, String search, String type) {
		if(type.equals("")){
		    return list(sortby, asc, search);
		}
		
		if(asc){
		    @SuppressWarnings("unchecked")
		    List<Org> listUser = (List<Org>) sessionFactory.getCurrentSession()
				.createCriteria(Org.class).add(Restrictions.eq("type",type))
				.add(Restrictions.ilike("name","%"+search+"%"))
				.addOrder( Order.asc(sortby) )
				.setResultTransformer(Criteria.DISTINCT_ROOT_ENTITY)
				.list();
